...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        '/questionnaire': (context) => QuestionnairePage(), // Add this line
        // Other routes...
      },
    );
  }
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _filterScoresForSelectedRange() {
    ...
    switch (_selectedRange) {
      case DateRange.week:
        startDate = now.subtract(Duration(days: now.weekday - 1)); // Start from the beginning of the week
        daysInRange = 7;
        break;
      case DateRange.month:
        startDate = DateTime(now.year, now.month, 1); // Start from the beginning of the month
        daysInRange = DateUtils.getDaysInMonth(now.year, now.month);
        break;
      case DateRange.year:
        startDate = DateTime(now.year, 1, 1); // Start from the beginning of the year
    ...
    endDate = startDate.add(Duration(days: daysInRange ));
    updateGraphData();
    setState(() {});
  }
  ...
  void updateGraphData() {
    _scoresToGraph = _healthScores.where((score) {
      ...
      return scoreDate.isAfter(startDate.subtract(Duration(days: 1))) && scoreDate.isBefore(endDate.add(Duration(days: 1)));
    }).map((score) {
      DateTime scoreDate = DateTime.fromMillisecondsSinceEpoch(score.x.toInt());

      // Correct calculation: daysSinceStart should be 0 for the start date
  ...
  @override
  Widget build(BuildContext context) {
    // Get the screen width and height to make the graph responsive
      ...
      body: SingleChildScrollView( // Wrap your column in a SingleChildScrollView
        child: Column(
          ...
          children: <Widget>[
            ...
            Container(
              width: screenWidth * 0.9, // Make the graph take up 90% of screen width
              height: screenHeight * 0.5, // Make the graph height dynamic based on screen height
              child: Padding(
                  ...
                  LineChartData(
                    ...
                    lineBarsData: [
                      LineChartBarData(
                        ...
                        isCurved: false, // Ensure the line is straight
                        color: Colors.blue,
                        barWidth: 2, // Adjust the bar width for better visibility
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            DateTime labelDate;
                            if (_selectedRange == DateRange.week) {
                              labelDate = startDate.add(Duration(days: value.toInt() + 1));
      ...
      drawer: Drawer(
        child: ListView(
          children: <Widget>[
            ...
            ListTile(
              title: const Text('Questionnaire'), // New menu option
              leading: Icon(Icons.question_answer),
              onTap: () {
                Navigator.pop(context); // Close the drawer
                Navigator.pushNamed(context, '/questionnaire'); // Navigate to the QuestionnairePage
              },
            ),
            ListTile(
